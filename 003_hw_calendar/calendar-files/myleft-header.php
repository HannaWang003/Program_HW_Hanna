<div class="container-fliud">
                <div class="row align-items-center w-75 m-auto">
                    ...
                    <div class="col-12 col-sm-6 text-start text-sm-end d-flex flex-nowrap myLheader-col-2">
                        <button class="col-4 ms-1 btn border-light myhover-1"><a class="text-decoration-none mytext-c2" href="calendar.php?prev=<?=$month?>&year=<?=$year?>">PREV</a></button>
                        <button class="col-4 ms-1 btn border-light myhover-1"><a class="text-decoration-none mytext-c2" href="calendar.php">||</a></button>
                        <button class="col-4 ms-1 btn border-light myhover-1"><a class="text-decoration-none mytext-c2" href="calendar.php?next=<?=$month?>&year=<?=$year?>">NEXT</a></button>
                    </div>
                ...
                <!-- 加入選擇日期功能 -->
                            <form class="d-flex flex-nowrap align-items-center" action="calendar.php" method="post">
                                ...
                                <?php
                                        for($i=0;$i<12;$i++){ 
                                            if($i+1==$month){
                                                echo "<option class='col-4 ms-1 text-end' value=".($i+1)." selected>".$_SESSION['monthS'][$i]."</option>";
                                            } 
                                            else{                                         
                                            echo "<option class='col-4 ms-1 text-end' value=".($i+1).">".$_SESSION['monthS'][$i]."</option>";
                                        } 
                                        }
                                    ?>
                                </select>
